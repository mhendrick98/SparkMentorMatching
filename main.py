...
def openMentorMatchingFellows(event):
    webbrowser.open('https://docs.google.com/spreadsheets/d/1QViaS9TlW-5rFkk8wAgwhehgtEIgZcZVEgDX_2qQSY8/edit#gid=2111577187')

def openMentorMatchingMentors(event):
    webbrowser.open('https://docs.google.com/a/bu.edu/spreadsheets/d/1fgKFACChlMfZYBV7PNitkIa9hyb3mqYlID7xxTrmUfk/edit?usp=forms_web_b#gid=314162645')
...
def main():
    ...
    row = Frame(root)
    Label(row, width=1000, text="", anchor='w').pack(side=LEFT)
    ...
    hyper_text = Label(root, text="Mentor Matching - Students (Responses)", fg="blue", cursor="hand2")
    hyper_text.pack(side=LEFT, padx=5, pady=5)
    hyper_text.bind("<Button-1>", openMentorMatchingFellows)

    hyper_text = Label(root, text="Mentor Matching - Mentor (Responses)", fg="blue", cursor="hand2")
    hyper_text.pack(side=LEFT, padx=5, pady=5)
    hyper_text.bind("<Button-1>", openMentorMatchingMentors)
